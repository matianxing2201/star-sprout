...
> **星芽 StarSprout** —— 面向 4–12 岁儿童的自主探索学习空间。
...
| 名字 | 是什么 | 出现在哪 |
| --- | --- | --- |
| **星芽 StarSprout** | **产品名** | 只有这几个品牌位：`index.html` 的标题与描述、`src/app/router/index.ts` 的 `BRAND`/`DEFAULT_TITLE`、`package.json`、README、家长端页眉、页脚签名 |
| **学习世界 LearningWorld** | **领域名词**：孩子在地图上逛的那些地方（阅读森林 · 数学城堡 · 科学实验室……） | 界面上约 14 处：「去过的学习世界」「回到学习世界」「探索了 N 个学习世界」 |

**不要把界面里的「学习世界」当成产品名去替换** —— 那些文案用的都是领域名词，语义正确。

**也不要把它改叫「星球」**：年级已经占了星球这个隐喻（成长阶梯上的年级星球），
两个都叫星球会撞。这里刻意区分：**年级是星球，星球上的地方是学习世界。**

`localStorage` 的命名空间是 `kids-world`（见 `src/shared/utils/storage.ts`）——
它是用户看不见的存储键前缀，**改名会让已有学习进度读不到**，因此保持不动。
