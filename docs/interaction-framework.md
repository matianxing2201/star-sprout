...
| `measure-stamp` | 盖量词印章 | 每个物品盖上的印章等于它的 `measureId` | 量词搭配、词语语感 |
...
学量词最难的是记住搭配，而记搭配靠的是**反复动手试**，不是选 A/B/C。
`measure-stamp` 把量词做成一枚可以拿起来的印章：

```
拿起「座」→ 盖到 🏰 上 → 印章砸下 + 墨迹扩散 + 溅出小星星
                        → 物品下方永久落下「一座城堡」
                        → 手里的印章自动放下（逼孩子为下一个重新判断）
全部盖对 → 所有短语飞进底部「朗读条」，连起来读一遍
```

它同时满足两条产品约束：**答错不摧毁进度**（物品原地保留、只给方向提示），
以及**奖励分级**（印章本身用 `fx-stamp` 这一档，真正的结算仍由外壳按等级播放）。

动画只在 `styles/animations.css` 里加了三个类（`fx-stamp` / `fx-ink` / `fx-spark`），
没有引入任何动画库 —— 与全站动画体系一致。
...
> 这条流程不是纸上谈兵：`measure-stamp`（量词印章）就是照它加进来的 ——
> 改 3 个文件（类型 / 注册表 / 一个组件）+ 1 条 schema，没有动任何页面。
