...
界面不再用 emoji 当图标（emoji 跨系统字形不同、自带颜色、粗细不一、基线难对齐，是界面显得生硬的主因）。
改用矢量图标 + 一层语义抽象：

- **词汇表在 domain**（`src/domain/shared/icons.ts`），**渲染在 ui**（`src/ui/icons/`）。
  两边靠 `satisfies Record<AppIconName, Component>` 对齐，写错名字或漏做映射都是**编译期错误**。
- **规则是「结构用图标，场景用 emoji」**：年级 / 学习世界 / 领域 / 主题 / 徽章 / 状态控件走矢量图标；
  互动场景里的实物（一只鸟、一根胡萝卜）仍是 emoji —— 它们本来就是插画。互动元素两种都可写，`icon` 优先。
- **三档字重**：控件用 `bold`，身份型图标用 `duotone`（纸雕的两层剪纸感），已获得的状态用 `fill`。
- `Category.icon` 是**可选**的：省略时按色调取默认图标，所以新领域不会出现一个空洞的图标位。
- **图标按需内联**：直接使用图标库的运行时组件包，121 个图标 × 6 种字重会让图标占掉首屏 JS 的
  163 kB gzip（72%）；改成只内联用得到的字重后降到约 41 kB gzip，首屏从 226 kB 降到 **152 kB gzip**。
- 开发环境访问 **`/dev/icons`** 可以看图挑名字（该路由只在 DEV 注册）。

详见 [`docs/design-system.md`](./docs/design-system.md) 第二节。
